...
@implementation MockPuzzleVerifyView
...
- (void)commonInit {
    ...
    // Init value
    kTTGPuzzleDefaultSize = CGSizeMake(40, 40);
    self.enable = YES;
    self.puzzlePattern = MockPuzzleVerifyClassicPattern;
...
- (UIBezierPath *)getNewScaledPuzzledPath {
    ...
    // Pattern path
    if (_puzzlePattern == MockPuzzleVerifyCustomPattern) {
        path = [UIBezierPath bezierPathWithCGPath:_customPuzzlePatternPath.CGPath];
        _puzzleSize = path.bounds.size;
    } else {
        path = [UIBezierPath bezierPathWithCGPath:[MockPuzzleVerifyView verifyPathForPattern:_puzzlePattern].CGPath];
...
- (void)setPuzzlePattern:(MockPuzzleVerifyPattern)puzzlePattern {
